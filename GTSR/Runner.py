...
import yaml
import shutil
from util import cal_psnr,cal_ssim,padding,inv_padding
...
class Runner:
    ...
    def train(self,args,train_loader,test_loader):
        ...
        for epoch_idx in range(last_epoch):
            ...
            for input_data in batch_list:
                ...
                if (current_idx) % self.config["val_freq"] == 0:
    ...
    def test(self,test_loader, save_img = False):
        if self.rank == 0:
            print("======= test ========")
            logging.info("======= test ========")
        ...
        with torch.no_grad():
            ...
            for batch_idx, input_data in enumerate(batch_list, 0):
                ...
                if save_img:
                    self.save_img(img=np.uint8(output[0,0].cpu().numpy() * 255), folder="Scale_%d" % self.config["datasets"]["scale"],
                                  name="%03d.png" % (batch_idx * dist.get_world_size() + self.rank))


                output = ((output[0] * 255).permute(1, 2, 0).cpu().numpy()).astype(np.uint8)
                HQ = ((HQ[0] * 255).permute(1, 2, 0).cpu().numpy()).astype(np.uint8)
                psnr = cal_psnr(HQ, output)
                ssim = cal_ssim(HQ, output)
