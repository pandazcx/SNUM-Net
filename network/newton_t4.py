...
class MRB(nn.Module):
    def __init__(self, dim, ratio_spatial, ratio_fusion, window_size = 32, sigma = 8):
        ...
        self.conv = nn.Conv2d(dim, dim * 2, kernel_size=1, padding="same", groups=1, bias=False)
        self.CPE = nn.Conv2d(dim, dim, kernel_size=3, padding="same", groups=1, bias=False)
        self.fusion = Fusion(dim,ratio_fusion)
    ...
    def forward(self, x):
        x_local,x_global = self.conv(x).chunk(2, dim=1)
        ...
        x = x + self.fusion(x_local,x_global)
        return x
...
class UNet(nn.Module):
    def __init__(self, in_dim, feature_dim, ratio_spatial, ratio_freq, ratio_fusion, middle_blk_num=2, enc_blk_nums=[1,1], dec_blk_nums=[1,1]):
        ...
        dim = dim * 2
        self.middle_blks = nn.Sequential(*[MRB(dim,ratio_spatial,ratio_fusion) for _ in range(middle_blk_num)])


        self.ups.append(nn.Sequential(nn.Conv2d(dim, dim * 2, kernel_size=3, bias=False,padding="same"), nn.PixelShuffle(2)))
        dim = dim // 2
        self.fusions.append(Fusion(dim,ratio_fusion))
...
class TBAF(nn.Module):
    ...
    def Visualization(self):
        grid = torch.linspace(-10, 10, 1000, requires_grad=True)
        grid_p = grid.unsqueeze(-1).repeat(1,self.control_nums)
        Fourier_sin = torch.sin(self.freq * grid_p)
        Fourier_cos = torch.cos(self.freq * grid_p)
        grid_p = torch.sum(self.weight_a * Fourier_sin + self.weight_b * Fourier_cos, dim=-1) + grid
        grid_p = grid_p.detach()
        grid = grid.detach()

        plt.plot(grid, grid_p, label='Cubic Spline Activation')
        plt.title('Cubic Spline Activation Function')
        plt.xlabel('x')
        plt.ylabel('y')
        plt.legend()
        plt.grid(True)
...
class Image_update(nn.Module):
    def __init__(self, in_dim,y_dim, feature_dim, ratio_spatial, ratio_freq, ratio_fusion, control_nums, middle_blk_num=2, enc_blk_nums=[1,1], dec_blk_nums=[1,1]):
        super(Image_update, self).__init__()
        self.matrix_D = CB(in_dim, ratio_spatial)
        self.matrix_Dt = CB(in_dim, ratio_spatial)
        self.matrix_E = CB(in_dim, ratio_spatial)
        self.matrix_Et = CB(in_dim, ratio_spatial)
        self.matrix_P = nn.Sequential(nn.Conv2d(y_dim, in_dim, kernel_size=1, bias=False, padding="same"),CB(in_dim, ratio_spatial))
        ...
        self.FCA = FCA(in_dim)
        self.solver = UNet(in_dim, feature_dim, ratio_spatial=1, ratio_freq=1, ratio_fusion=ratio_fusion,
    ...
    def forward(self,u,z,y,l1,l2):
        ...
        l2_n = u - self.s_psi(self.FCA((l2 + u),self.conv(y))) + l2
        z_n = self.matrix_Dt(self.matrix_D(u)-z)
        y_n = self.matrix_Et(self.matrix_E(u-self.matrix_P(y)))
...
class Lambda_update(nn.Module):
    def __init__(self, in_dim,y_dim, ratio_spatial,control_nums):
    ...
    def forward(self,u,y,l1,l2):
        ...
        delta = u - self.s_psi(self.FCA((l2 + u),self.conv(y)))
        l2 = self.updater_2(l2, delta)
        return l1, l2


class DeepMSN_Net(nn.Module):
    def __init__(self, args):
        super(DeepMSN_Net, self).__init__()
        ...
        self.image_updaters = nn.ModuleList(nn.Sequential(*[Image_update(self.in_dim, self.y_dim, self.feature_dim, self.ratio_spatial, self.ratio_freq,
                                                                         self.ratio_fusion, self.control_nums, self.middle_blk_num, self.enc_blk_nums,
                                                                         self.dec_blk_nums) for _ in range(self.depth)]))
        self.lambda_updaters = nn.ModuleList(nn.Sequential(*[Lambda_update(self.in_dim, self.y_dim, self.ratio_spatial, self.control_nums) for _ in range(self.depth - 1)]))
        self.init = nn.Conv2d(self.in_dim, self.in_dim * 2, kernel_size=3, bias=False, padding="same")

    def forward(self, z,y):
        # y = torch.randn(1,3,256,256)
        # z = F.interpolate(z, scale_factor=self.scale_factor)
        u = z
...
if __name__ == '__main__':
    ...
    net = DeepMSN_Net(args)
    # help(get_model_complexity_info)
    inpz = torch.randn(1,1,256,256)
    inpy = torch.randn(1,3,256,256)
